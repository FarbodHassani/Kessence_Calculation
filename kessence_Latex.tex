...
\be
g_{\mu\nu}=-e^{2\Phi}dt^{2}+a^{2}\left( t\right) e^{-2\Psi}dr^{2}
\ee
\be
\delta g^{(1)}_ { 00}=-2\Phi \, \; \; , 
\delta g^{(1)}_{ij}= -2 a^{2} \Psi \delta_{ij}
\ee
Where $\delta g^{(1)}_ { 00}$ means the first order metric in pertubations.  The inverse of metric is defined as following,
\be
g^{\mu\nu}=-e^{-2\Phi}dt^{2}+a^{-2}e^{2\Psi}dr^{2} 
\ee
\be
\delta g_{(1)}^{00}=+2\Phi \, \; \; , 
\delta g_{(1)}^{ij}= +2 a^{-2} \Psi \delta^{ij} 
\ee
We have,
\be
...
\ee
\be
 \delta X_{1}=-\Phi+\dot{\pi}- \frac{1}{2 a^2} (\vec{\nabla} \pi)^2 +O\left( \varepsilon^{2}\right)
\ee
...
&+
  3 M_{pl}^2 H^2 \Omega  \Bigg[ - (1+\omega) \left (-\Phi+\partial_{t}\pi- \frac{1}{2 a^2} (\vec{\nabla} \pi)^2 \right )
 -
 2\Phi  \omega
   +
 2 (1+\omega) \dot{\pi}
...
\frac{2c_s^2-1}{2 a^2} (\vec{\nabla} \pi)^2 \right ] \Bigg]
 \nonumber \\ &
  = 3 M_{pl}^2 H^2 \Omega    \Bigg[ 1
+ \left (2-(1+\omega)  c_s^{-2}  \right )\Phi +
...
\begin{align}
& \ddot{\pi} ^{n} =- \frac{\pi^{n}_{i-1,j,k}+\pi^{n}_{i+1,j,k} +\pi^{n}_{i,j-1,k} +\pi^{n}_{i,j+1,k}+\pi^{n}_{i,j,k-1}+\pi^{n}_{i,j,k+1} -6 \pi^{n}_{i,j,k}  }{ a^2 dx^2}   +3 \frac{\Phi ^{n}_{i,j,k} -\Phi^{n-1}_{i,j,k} }{d t}    \nonumber \\ & + \frac{(3 H^2+2 \dot{H}) (1+3 w) }{H (1+w)} \Phi^{n}_{i,j,k} + H  \frac{(\pi_{v  \; {i,j,k}}^{n+\frac{1}{2}} +\pi_{v \; {i,j,k}}^{n-\frac{1}{2}} )}{2} -  \frac{ 6H w}{ 1+w} \Psi ^{n}_{i,j,k}
\end{align}
...
\begin{align} 
 &\pi_v ^{n+\frac{1}{2}}=\pi_v ^{n-\frac{1}{2}} +d t  \Big [c^2 \frac{\nabla^2 \pi ^n}{a^2} +3 \frac{\Phi ^{n}_{i,j,k} -\Phi^{n-1}_{i,j,k} }{d t} + \frac{(3 H^2+2 \dot{H}) (1+3 w) }{H (1+w)} \Phi^{n}_{i,j,k} + H  \frac{(\pi_{v  \; {i,j,k}}^{n+\frac{1}{2}} +\pi_{v \; {i,j,k}}^{n-\frac{1}{2}} )}{2} -  \frac{ 6H w}{ 1+w} \Psi ^{n}_{i,j,k} \Big]
\end{align}
So the set of equations we should solve are;
\begin{align} 
 \pi_v ^{n+\frac{1}{2}}= & \frac{1}{1- dt H/2} \Bigg[ (1+ dt H/2)\pi_v ^{n-\frac{1}{2}} + c^2 d t  \Big (  \frac{\pi^{n}_{i-1,j,k}+\pi^{n}_{i+1,j,k} +\pi^{n}_{i,j-1,k} +\pi^{n}_{i,j+1,k}+\pi^{n}_{i,j,k-1}+\pi^{n}_{i,j,k+1}  -6 \pi^{n}_{i,j,k}  }{ a^2 dx^2}  \nonumber \\ & +3 \frac{\Phi ^{n}_{i,j,k} -\Phi^{n-1}_{i,j,k} }{d t} + \frac{(3 H^2+2 \dot{H}) (1+3 w) }{H (1+w)} \Phi^{n}_{i,j,k} -  \frac{ 6H w}{ 1+w} \Psi ^{n}_{i,j,k} \Big) \Bigg]
\end{align}
\be
\pi^{n+1}= \pi ^{n}+\pi_v ^{n+\frac{1}{2}} d t
\ee
Then we need to calculate $\dot{H}$ and $\dot{\Phi}$ in each loop. \\
To calculate $\dot{\Phi}$ we save two $\Phi$ in each loop. \\
